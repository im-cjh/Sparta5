...
const handlerMappings: Record<ePacketId, PacketHandler> = {
  ...
  [ePacketId.B2L_Init]: (buffer: Buffer, session: LobbySession) =>
    defaultHandler(buffer, session),

  [ePacketId.L2C_GameStart]: (buffer: Buffer, session: LobbySession) =>
    defaultHandler(buffer, session),
  [ePacketId.L2B_Init]: (buffer: Buffer, session: LobbySession) =>
    defaultHandler(buffer, session),
  [ePacketId.C2B_Init]: (buffer: Buffer, session: LobbySession) =>
    defaultHandler(buffer, session),
  [ePacketId.B2C_Init]: (buffer: Buffer, session: LobbySession) =>
    defaultHandler(buffer, session),
};
